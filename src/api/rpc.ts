...
export class RPC {
  constructor(readonly provider: HTTPProvider) {}
  async getNodeInfo() {
    const url = 'getNodeInfo';
    return await this.provider.get<NodeInfo>(url);
  }
  async getChainInfo() {
    const url = 'getChainInfo';
    return await this.provider.get<ChainInfo>(url);
  }
  async getGasRatio() {
    const url = 'getGasRatio';
    return await this.provider.get<RPCResponse.GasRation>(url);
  }
  async getRAMInfo() {
    const url = 'getRAMInfo';
    return await this.provider.get<RAMInfo>(url);
  }
  async getTxByHash(hash: string) {
    const url = `getTxByHash/${hash}`;
    return await this.provider.get<TxInfo>(url);
  }
  async getTxReceiptByTxHash(hash: string) {
    const url = `getTxReceiptByTxHash/${hash}`;
    return await this.provider.get<TxReceiptInfo>(url);
  }
  async getBlockByHash(hash: string) {
    const url = `getBlockByHash/${hash}/true`;
    return await this.provider.get<BlockInfo>(url);
  }
  ...
  async getContractStorage(
    ...
    return await this.provider.post<RPCResponse.ContractStorage>(url, data);
  }
  async getContractStorageFields(
    ...
    return await this.provider.post<RPCResponse.ContractStorageFields>(
      url,
      data,
    );
  }
  async getBatchContractStorage(
    ...
    const res = await Promise.all(
      chunkedQuery.map((query) =>
        this.provider.post<RPCResponse.ContractStorages>(url, query),
      ),
    );
    return res;
  }
  async sendTx(tx: Transaction) {
    const url = 'sendTx';
    return await this.provider.post<RPCResponse.TransactionPending>(
      url,
      tx.toString(),
    );
  }
  async execTx(tx: Transaction) {
    const url = 'execTx';
    return await this.provider.post<TxReceiptInfo>(url, tx.toString());
  }
  subscribe(topics: Params.SubscribeEventType[], contract: string) {
    ...
    const event: StrictEventEmitter<EventEmitter, Params.SubscribeEvent> =
      new EventEmitter();
    this.provider.stream<ReadStream>(url, data).then(async (readable) => {
  ...
  async getToken721Balance(
    ...
    return await this.provider.get<RPCResponse.Token721Balance>(url);
  }
  async getToken721Metadata(
    ...
    return await this.provider.get<RPCResponse.Token721Metadata>(url);
  }
  async getToken721Owner(
    ...
    return await this.provider.get<RPCResponse.Token721Owner>(url);
  }
  async listContractStorage(
    ...
    return await this.provider.post<RPCResponse.ContractStorageList>(url, {
      id: contract,
      limit,
